...
// DASHBOARD
const inventoryTotalContainer = document.querySelector(
  ".inventory-total-container"
);
const inventoryTotal = document.querySelector(".inventory-total");
const inventoryTotalCompact = document.querySelector(
  ".inventory-total_compact"
);
const inventoryLow = document.querySelector(".inventory-low");
const inventoryLowCompact = document.querySelector(".inventory-low_compact");
const diffItems = document.querySelector(".different-items-total");
const diffItemsCompact = document.querySelector(
  ".different-items-total_compact"
);
const inventoryOOS = document.querySelector(".out-of-stock-item");
const inventoryOOSCompact = document.querySelector(
  ".out-of-stock-item_compact"
);
...
const manageOrderButton = document.querySelector(".manage-order");
const manageOrder = document.querySelector(".manage-order-container");
...
const addOrdersModul = document.querySelector(".new-order-container");
...
const pastOrderContainer = document.querySelector(
  ".child-orders-container_past"
);
...
class Master {
  ...
  constructor() {
    signInButton.addEventListener("click", this.signIn.bind(this));
    logoutIcon.addEventListener("click", this.signOut.bind(this));
  ...
  signIn() {
    ...
    /*  Un-comment this and remove above code to toggle easy login
    if ( 
      emailInput.value &&
  ...
  updateDashboard() {
    ...
    for (const day of weekdaysArr) {
      ...
      if (dayID !== today.getDay().toString()) {
        ...
        letter.style.color = "rgb(143, 143, 143)";
        number.style.color = "rgb(143, 143, 143)";
    ...
    for (const order of this.#orders) {
      ...
      if (new Date(order.deliveryDate).getTime() > now) {
        ...
        const html = `
        <div class="next-del-day-cont">
          <p class="next-del-day">${deliveryArriving}</p>
        </div> 
      <div class="next-del-parent">
      <div class="next-del-info">
        ...
        <div class="next-del-date-container">
          ...
          <p class="next-del-qty next-del-stat">${order.qty}<p>items</p></p>
        </div>
      </div>
      <div class="next-del-info_2">
  ...
  togglePages(e) {
    ...
    for (const item of sidebarContainer.children) {
      item.style.backgroundColor = "#264c6b";
    }

    const selectedTab = document.querySelector(`.${target}`);

    selectedTab.style.backgroundColor = "#426785";

    const selectedScreen = document.querySelector(`.${target}-container`);
  ...
  addInventoryToDOM(item) {
    ...
    const html = `<div class="inventory-row-container" data-id="${item.upc}">
    ...
    inventoryContent.insertAdjacentHTML("beforeend", html);
  ...
  addStoreToDOM(store) {
    ...
    const html = `
    <div class="new-store-container new-store-container_${store.storeNumber}">
      ...
      for (let x = 0; x < searchInputValue.length; x++) {
        ...
        for (let q = 0; q < objNames.length; q++) {
          ...
          if (objNames[q].terminate === false) {
            console.log(`evaluating ${objNames[q].name} & ${objNames.vendor}`);
  ...
  addOrdersToDOM(order) {
    ...
    // RECEIVE ORDER //////////////
    const receiveOrderButton = document.querySelector(
      `.receive-order-button_${order.uniqueOrderCode}`
    );
    receiveOrderButton.addEventListener(
      "click",
